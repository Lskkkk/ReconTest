...
const init = async () => {
	...
	const allPros = files.map(fileName => {
		...
		return workbook.xlsx.readFile(path.join(fileDir, fileName)).then(() => {
			...
			worksheet.eachRow((row, rowNumber) => {
				...
				if (!isFundNameValid(cnName)) return;
				if (!allData[cnName]) {
					allData[cnName] = {};
				}
	...
	Object.keys(allData).forEach(id => {
		const values = [];
		const dateList = Object.keys(allData[id]).sort((a, b) => (compareDate(a, b) ? -1 : 1));
...
const getCache = id => {
	...
	if (originalData[id]) {
		...
		if (lastRequest && lastRequest[id] && lastRequest[id] == formatDate(new Date())) {
...
const getCacheFundExtraOptions = () => {
	...
	if (Object.keys(originalData).length > 0) {
		return Object.keys(originalData).filter(key => isFundNameValid(key));
	}
	return [];
};
